...
**Decision:** "Premium through craft" — no signature hue. Three-tier typography (Rigid Square headings, Titillium Web body, Source Code Pro data), slot-roll billboard number animation (digits translate up into masked containers with per-digit stagger), staggered card entrance animations, and a shared spring easing variable. Network Health page reframe of the API status page. Credits flow showcase with total ALEPH counter (ℵ prefix) and watermark. Semantic colors (green/red/amber) stay pure for status — brand identity comes from motion choreography and data visualization quality.
**Rationale:** Research across RunPod, Vercel, Railway, DigitalOcean, Cloudflare, Datadog, and Linear showed that every premium dashboard has a visual signature. The dashboard's distinctive elements are the credits flow particle animation and glassmorphism stat cards — not a color. Forcing a brand color onto status indicators creates tension (is green "healthy" or "brand"?). The DS lime accent (`#D4FF00`) was considered but rejected — it conflicts with semantic success green and the dashboard uses it in only 3 places. Slot-roll chosen over simple count-up because the billboard effect creates a stronger first impression. Motion applied only on first load (not polling updates) to avoid feeling sluggish.
**Alternatives considered:** Lime accent as signature color (conflicts with status green), purple-to-lime gradient (adds complexity), simple count-up animation (less distinctive than slot-roll billboard)
