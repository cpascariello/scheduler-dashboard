...
- Credits page: `/credits` with 24h/7d/30d range selector, credit expense distribution flow diagram (SVG animated paths), summary stat cards (total/storage/execution/dev fund), recipient table with role badges (CRN/CCN/staker), sidebar nav entry with coins icon. Uses api2 credit expense messages + corechannel aggregate for node state. Distribution logic: 5% dev fund, CRN share (60% execution), CCN share (75% storage / 15% execution, score-weighted), staker share (20%, stake-weighted). Shared React Query cache for 24h data across credits and wallet pages via stable 5-minute-rounded timestamps.
