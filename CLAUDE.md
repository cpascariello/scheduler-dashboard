...
- Three-tier typography: Rigid Square headings (Typekit), Titillium Web body (Typekit), Source Code Pro data (`--font-mono` override). Slot-roll billboard animation on overview stat numbers and credits total (digits translate up into `overflow-hidden` containers, staggered per digit). Staggered card entrance on overview page. Shared `--ease-spring` easing. Respects `prefers-reduced-motion`.
- Network Health page (`/status`): hero banner with "All Systems Operational" / degraded pill (green/red glow), quick stats bar (endpoints healthy, avg latency, active nodes, running VMs from cached `useOverviewStats`), Scheduler API + Aleph API endpoint sections with StatusDot/HTTP code/latency, auto-refresh every 60s, recheck button, `?api=` URL override
