...
- API status page: two sections — Scheduler API (`/health` + 7 v1 endpoints) and Aleph API (messages, aggregates, authorizations); per-section summary ring (healthy/total donut), response latency (ms), HTTP codes, StatusDot health, staggered fade-in animation, auto-refresh every 60s with last-checked timestamp, recheck button, `?api=` URL override for scheduler base URL
